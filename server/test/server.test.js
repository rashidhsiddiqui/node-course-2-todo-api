...
//Now seeded todos and populateTodos are coming from seed.js
...
describe("GET /todos", () => {
  ...
  it("should return all todos", (done) => {
    ...
    .expect(200)
    .expect((res) => {
      expect(res.body.todos.length).toBe(1); //1 is set for user[0] created todo is 1
    })
    .end(done);
  });
});
...
describe("DELETE todos/:id", () => {
  ...
  it("should not remove a todo", (done) => {
    ...
    var hexId = todos[0]._id.toHexString(); //delete the second todo from array
...
describe("GET /users/me", () => {
  ...
  it("should return 401 if user is non authenticated", (done) => {
    request(app)
      .get("/users/me")
      //.set("x-auth", "1234")
      .expect(401)
       .expect((res) => {
         expect(res.body).toEqual({}) //expect empty user body
...
describe("POST /users", () => {
  ...
  it("should create a new user", (done) => {
      ...
      .expect(200)
      .expect((res) => {
        expect(res.header["x-auth"]).toExist(); //in response x-auth should be return of newly created user
  ...
  it("should return validation errors if request invalid", (done) => {
    request(app)
    .post("/users")
    .send({email: "newEmail", password: "newPa"})
    .expect(400)
    .end(done);
  });

  it("should not create a user if user is in use", (done) => {
    request(app)
    .post("/users")
...
describe("POST /users/login", () => {
  ...
  //var email = "talha@test.com";
  //var password = "talhaPass";
  ...
  it("should reject invalid login", (done) => {
    request(app)
    .post("/users/login")
    .send({email: users[1].email, password: "asasa"})
    .expect(400)
    .expect((res) => {
    ...
    .end((err, res) => {
      ...
      User.findById(users[1]._id).then((user) => {
        //expect(user.tokens[0]).toEqual(undefined)
